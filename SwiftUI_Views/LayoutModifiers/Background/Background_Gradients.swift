...
struct Background_Gradients: View {
    ...
    var body: some View {
        ...
        return VStack(spacing: 10) {
            ...
            Text("Using Gradients").foregroundStyle(.gray)
            ...
            Button(action: {}) {
                Text("Button").padding().foregroundStyle(.white).padding(.horizontal, 20)
            }
            .background(linearGradient)
            .cornerRadius(15)
            ...
            HStack() {
                Text("This is in an HStack").padding().foregroundStyle(.white)
            }
            .frame(maxWidth: .infinity)
            .background(linearGradient)
        }
